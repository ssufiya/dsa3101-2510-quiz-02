...
# Prefer a sentinel that only the Python seeder writes
if docker exec "${CONTAINER}" psql -U postgres -d quizbank -tAc \
   "SELECT 1 FROM pg_tables WHERE tablename='system_meta';" | grep -q 1; then
  # system_meta table exists → check the sentinel row
  if docker exec "${CONTAINER}" psql -U postgres -d quizbank -tAc \
    ...
    echo "⚙️ Running initial database seeding (no sentinel found)..."
    python "${INIT_SCRIPT}"
    echo "💾 Creating baseline backup after first initialisation..."
    bash "${BACKUP_SCRIPT}"
  fi
else
  # system_meta not present yet → definitely first run
  echo "⚙️ Running initial database seeding (first run)..."
  python "${INIT_SCRIPT}"
  echo "💾 Creating baseline backup after first initialisation..."
  bash "${BACKUP_SCRIPT}"
fi
