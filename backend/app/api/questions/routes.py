...
STORAGE_OTHER_PATH.mkdir(parents=True, exist_ok=True)
...
def get_storage_path_for_file(filename: str) -> Path:
    ...
    # Save both images and R files to PNG storage
    if ext in IMAGE_EXTENSIONS or ext in {'.r', '.rmd'}:
        return STORAGE_PNG_PATH
    else:
        return STORAGE_OTHER_PATH
...
def copy_attachments_to_storage(attachment_paths: Dict[str, str]) -> int:
    ...
    for att_name, att_path in attachment_paths.items():
        try:
            ...
            if file_ext not in IMAGE_EXTENSIONS and file_ext not in {'.r', '.rmd'}:
                logger.info(f"⏭️ Skipping non-image/non-R file: {att_name} ({file_ext})")
                continue
            
            # Determine target directory based on file type (images and R files go to PNG storage)
            target_dir = get_storage_path_for_file(att_name)
            target_dir.mkdir(parents=True, exist_ok=True)
            target_path = target_dir / att_name
            ...
            if file_ext in IMAGE_EXTENSIONS:
                logger.info(f"✅ Copied image: {att_name} → {target_path}")
            else:
                logger.info(f"✅ Copied R file: {att_name} → {target_path}")
